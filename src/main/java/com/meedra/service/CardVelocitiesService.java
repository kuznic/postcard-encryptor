...
import com.meedra.repository.CardVelocitiesRepository;
...
@Component
@Service
public class CardVelocitiesService {

    @Autowired
    Gson gson;
    @Autowired
    private CardVelocitiesRepository cardVeloRepo;
    @Autowired
    ...
    public HashMap fetchCards() {
        ...
        List<String> cards = cardVeloRepo.getAllCards(issuer_nr);
    ...
    public void updateCards(HashMap<String, Object> encCardIdsMap) throws Exception {
        ...
        for (Object o : encCardIdsMap.entrySet()) {
            ...
            val cardToBeUpdated = cardVeloRepo.fetchCardVelRecord(mapElement.getKey().toString());
            if (cardToBeUpdated == null) {
                throw new Exception("No record found for CardId " + mapElement.getKey().toString());
            }

            numberOfRecordsUpdated = cardVeloRepo
                    .updateCardVelocityRecord(mapElement.getKey().toString(), mapElement.getValue().toString()) + numberOfRecordsUpdated;
