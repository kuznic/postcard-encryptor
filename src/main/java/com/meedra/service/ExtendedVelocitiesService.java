...
import com.meedra.repository.ExtendedVelocitiesRepository;
...
@Component
@Service
public class ExtendedVelocitiesService {

    @Autowired
    Gson gson;
    @Autowired
    private ExtendedVelocitiesRepository extVeloRepo;
    @Autowired
    ...
    public HashMap fetchCards() {
        ...
        List<String> cards = extVeloRepo.getAllCards(issuer_nr);
    ...
    public void updateCards(HashMap<String, Object> encCardIdsMap) throws Exception {
        ...
        for (Object o : encCardIdsMap.entrySet()) {
            ...
            val cardToBeUpdated = extVeloRepo.fetchExtendedVelRecord(mapElement.getKey().toString());
            if (cardToBeUpdated == null) {
                throw new Exception("No record found for CardId " + mapElement.getKey().toString());
            }

            numberOfRecordsUpdated = extVeloRepo
                    .updateExtendedVelocityRecord(mapElement.getKey().toString(), mapElement.getValue().toString()) + numberOfRecordsUpdated;
